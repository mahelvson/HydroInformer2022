...
class Exp_Informer(Exp_Basic):
    ...
    def _get_data(self, flag):
        ...
        data_dict = {
            'camels_ds': DynamicStaticDataset, # camels dynamic + static
            'camels_d':  DynamicDataset, # camels dynamic
            }
        ...
        if self.args.data == 'camels_d':
            data_set = Data(
                ...
                data_path_dynamic=args.data_path_dynamic,
                flag=flag,
            ...
            data_set = Data(
                root_path=args.root_path,
                data_path_dynamic = args.data_path_dynamic,
                data_path_static = args.data_path_static,
                flag=flag,
                size=[args.seq_len, args.label_len, args.pred_len],
                features=args.features,
                target=args.target,
    ...
    def _process_one_batch(self, dataset_object, batch_x, batch_y, batch_x_mark, batch_y_mark, static_attr = None):
        ...
        if self.args.use_amp:
            with torch.cuda.amp.autocast():
                if self.args.output_attention:
                    outputs = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark, static_attr)[0]
                else:
                    outputs = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark, static_attr)
        else:
            if self.args.output_attention:
                outputs = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark, static_attr)[0]
            else:
